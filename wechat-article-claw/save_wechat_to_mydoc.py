...
"""
抓取单篇微信公众号文章 → my-wiki：
  01_mydoc/14_feeds/{年}/{标题}.md
  _assets/images/14_feeds/wechat/{slug}/图片.png

正文范围：仅 #js_content；过滤广告 iframe、推广块、二维码尾图等。
"""
...
def slugify(text: str, max_len: int = 48) -> str:
    text = unicodedata.normalize("NFKC", text or "wechat-article").strip()
    text = re.sub(r'[\\/:*?"<>|]', "-", text)
...
def render_image_markdown(
    ...
    *,
    ...
    if is_ad_image(img):
        return ""
    url = url_override or resolve_img_url(img)
    if not url:
        return ""
...
def linear_content_blocks(root: Tag):
    """按 DOM 顺序线性遍历，避免 figure/嵌套 section 漏图。"""
...
def download_image(fetcher: WechatArticleFetcher, url: str, dest: Path) -> Path | None:
    """Download image; return the path actually written (suffix may differ from dest)."""
...
def main() -> int:
    cli = argparse.ArgumentParser(description="Save WeChat article to my-wiki 14_feeds + _assets.")
    cli.add_argument("url", help="https://mp.weixin.qq.com/s/...")
    cli.add_argument(
        "--vault",
        default=os.environ.get("CSLOGIN_WIKI_PATH", ""),
        help="my-wiki vault root (default: CSLOGIN_WIKI_PATH)",
    )
    cli.add_argument("--tag", action="append", default=[], dest="tags", help="Extra frontmatter tag")
    cli.add_argument("--dry-run", action="store_true")
    cli.add_argument("--force", action="store_true", help="Overwrite existing md and refresh assets")
            ...
            json.dumps(
                {
                    ...
                    "message": "Set --vault or CSLOGIN_WIKI_PATH to my-wiki root.",
                },
                ensure_ascii=False,
            ),
            file=sys.stderr,
